...
function html_top_banner() {
	...
	if( !is_blank( $t_page ) && file_exists( $t_page ) && !is_dir( $t_page ) ) {
		...
		$LogoFloat = should_center_logo() ? '' : 'style ="float: left;"';
    

		echo '<div align="', $t_align, '" ' . $LogoFloat . '>';
		if( $t_show_url ) {
...
function html_login_info() {
	...
	echo '<table class="hide dajo_header">';
	echo '<tr>';
	...
	if( current_user_is_anonymous() ) {
		...
		echo lang_get( 'logged_in_as' ), " <br /><span class=\"italic dajo_span_logged_in_name\">", string_html_specialchars( $t_username ), "</span> <span class=\"dajo_span_logged_in_name\">";
		echo is_blank( $t_realname ) ? "($t_access_level)" : "(" . string_html_specialchars( $t_realname ) . " - $t_access_level)";
		echo "</span>";
	}
	echo '</td></tr>';
	echo '<tr><td class="login-info-middle">';
	echo "<span class=\"italic\">$t_now</span>";
	echo '</td></tr>';
	echo '<tr><td class="login-info-right">';
	...
	if( $t_show_project_selector ) {
		...
		echo '<span class="dajo_project_label">' . lang_get( 'email_project' ), ': </span>';
		if( ON == config_get( 'show_extended_project_browser' ) ) {
			print_extended_project_browser( helper_get_current_project_trace() );
		} else {
			if( ON == config_get( 'use_javascript' ) ) {
				echo '<select name="project_id" class="dajo_project_select" onchange="document.forms.form_set_project.submit();">';
			} else {
				echo '<select name="project_id" class="dajo_project_select">';
			}
			print_project_option_list( join( ';', helper_get_current_project_trace() ), true, null, true );
			echo '</select> ';
		}
		echo '<input type="submit" class="dajo_project_submit" value="' . lang_get( 'switch' ) . '" />';
		echo '</form>';
	} else {
	...
	if( OFF != config_get( 'rss_enabled' ) ) {
		...
		echo '<img src="' . helper_mantis_url( 'images/dajo_custom/dajo_rssicon.png' ) . '" alt="' . lang_get( 'rss' ) . '" class="dajo_rss_img" />';
		echo '</a>';
	}
...
function html_footer( $p_file = null ) {
	...
	echo "\t", '<br />', "\n";
	echo "\t", '<hr class="dajo_hr" />', "\n";

	echo '<table class="dajo_footertable" border="0" width="100%" cellspacing="0" cellpadding="0"><tr valign="top"><td>';
...
function print_menu() {
	if( auth_is_user_authenticated() ) {
		...
		echo '<table class="dajo_menu" cellspacing="0">';
		echo '<tr>';
		echo '<td>';
		...
		echo '</td>';
		echo '<td class="dajo_menu_jump menu right nowrap">';
		echo '<form method="post" action="' . helper_mantis_url( 'jump_to_bug.php">' );
		# CSRF protection not required here - form does not result in modifications

		if( ON == config_get( 'use_javascript' ) ) {
			$t_bug_label = lang_get( 'issue_id' );
			echo "<input type=\"text\" name=\"bug_id\" size=\"10\" class=\"dajo_jump-input-small\" value=\"$t_bug_label\" onfocus=\"if (this.value == '$t_bug_label') this.value = ''\" onblur=\"if (this.value == '') this.value = '$t_bug_label'\" />&#160;";
		} else {
			echo "<input type=\"text\" name=\"bug_id\" size=\"10\" class=\"small\" />&#160;";
		}

		echo '<input type="submit" class="dajo_jump-button-small" value="' . lang_get( 'jump' ) . '" />&#160;';
		echo '</form>';
		echo '</td>';
		echo '</tr>';
		echo '</table>';
...
function print_summary_submenu() {
	echo '<div align="center" class="dajo_adminmenu_line">';
